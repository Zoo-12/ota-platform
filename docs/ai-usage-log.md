...
### 활용 내용: 외부 공급사 예약 지원 / ERD·아키텍처 문서 정비 / 테스트 보강 / SAGA 패턴 문서화 / 외부 예약 취소 기능

**AI에게 요청한 내용:**
- 프론트엔드 API 응답 불일치 수정 (bookingKey, source, createdAt 등 필드 누락)
- 외부 공급사 예약 지원: `external_booking` 테이블 신규 생성 (Flyway V3), 예약 API 재사용
- `ExternalBooking` 엔티티, `ExternalBookingRepository`, `CreateExternalBookingUseCase` 구현
- 컨트롤러 외부 예약 생성 로직을 `CreateExternalBookingUseCase`로 분리
- 예약 키 prefix(`INT-`/`EXT-`)를 `BookingKeyType` 열거형으로 상수화
- `GetBookingDetailUseCase`에 내부+외부 예약 통합 조회 구현
- ERD 문서에 `external_booking` 테이블 및 Mermaid 다이어그램 추가
- `BookingKeyType` 단위 테스트, `ExternalBookingIntegrationTest` 통합 테스트 작성
- 아키텍처 문서에 SAGA 패턴 적용 계획 및 FeignClient 연동 방향 문서화
- `ExternalBookingStatus` 열거형 도입 (String → enum 타입 변환, DB CHECK 제약 추가)
- `CancelExternalBookingUseCase` 구현 (취소 시 status CANCELLED 전이)
- 프론트엔드 외부 예약 취소 버튼 활성화 및 빌드/배포

**본인 판단 및 수정 사항:**
- 외부 공급사 예약을 별도 엔드포인트로 분리하지 않고 기존 `POST /api/customer/bookings` 재사용 방향 직접 결정 — `accommodationId` prefix로 내부/외부 분기
- SAGA 패턴을 실제 구현하지 않고 주석·문서로만 의도를 표현하는 방향 직접 결정 — 현재 단계에서 코드 복잡도를 높이는 것보다 설계 의도 기록이 더 가치 있다고 판단
- `ExternalBookingStatus` 타입 변환 시 Flyway 마이그레이션이 필요한지 판단 — VARCHAR(20)에 EnumType.STRING을 사용하면 구조적 변경 없이 동작하지만, CHECK 제약으로 허용값을 명시적으로 제한하는 V4 마이그레이션 추가 (DB 안전성 강화)
- 외부 예약 취소 시 사유(reason) 입력 프롬프트를 내부 예약에만 표시하도록 프론트 수정 직접 판단
