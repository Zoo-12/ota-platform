...
RequestLoggingFilter
    ...
    └── 응답 헤더 X-Trace-Id 로 반환  → 개발 편의
```

에러 응답 body에도 `traceId`를 포함시켜, 클라이언트가 서버 로그와 연결할 수 있도록 한다.

> **참고:** 현재 구현에서는 개발 및 테스트 편의를 위해 응답 헤더(`X-Trace-Id`)와 에러 body에 `traceId`를 노출하고 있다.
> 실제 운영 환경에서는 보안 관점에서 `traceId`를 클라이언트에 직접 노출하지 않고, 서버 내부 로깅 용도로만 사용하는 것이 권장된다.
