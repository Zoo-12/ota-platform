...
- 동일 객실의 동시 예약은 빈도가 낮아 락 경합 비용이 낮음
...
대규모 동시 요금 조회를 처리하기 위해 Redis 캐싱을 적용한다.

```
캐시 키: rate:{roomTypeId}:{date}
TTL: 5분
캐시 미스 시: DB 조회 후 캐시 저장
```

파트너가 요금을 수정하면 해당 키 캐시를 무효화(evict)한다.

**캐싱 적용 범위:**
- 요금 조회 (DailyRate) — 캐시 적용
